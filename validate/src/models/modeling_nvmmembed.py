...
class NVMMEmbedModel(LlavaNextForConditionalGeneration):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            for_inputs_embeds_ids[
                (input_ids == 32001)] = 2  # We use tokenizer from Llava-Next but later replace PAD with EOS Token
    ...
    def encode(self, inputs, is_query=False, instruction=None, max_length=512, query_prefix='Query: '):
        ...
        if len(imgs) == 0:
            imgs = None
        collated_features = self.preprocess_fn(prompts, imgs, return_tensors="pt", padding="longest",
        ...
        instruction_lengths = self.calculate_instruction_length(self.preprocess_fn.tokenizer, prompts,
                                                                f'\n{query_prefix}')
