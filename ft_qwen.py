...
# os.environ["SWANLAB_PROJECT"] = "qwen-qlora-embedding"
...
def create_qwen_prompt(text_prompt):
    """为Qwen模型创建正确格式的prompt"""
    template = qwen2_5_vl_template.copy()
    template[0]["content"][0]["text"] = text_prompt
    return template

def process_qwen_template(template, text):
    """处理Qwen模板，替换占位符"""
    if isinstance(template, list):
        # 如果是列表格式，直接修改text内容
        processed_template = template.copy()
        original_text = processed_template[0]["content"][0]["text"]
        # processed_text = original_text.replace("*sent_0*", text).strip()
        processed_text = original_text.replace("*sent_0*", text)
        processed_text = processed_text.replace('_', ' ').replace('*sep+*', '') \
            .replace('*cls*', '').replace('\\n', '\n')
        processed_template[0]["content"][0]["text"] = processed_text
        return processed_template
    else:
        template = template.replace('_', ' ').replace('*sep+*', '')\
                                             .replace('*cls*', '').replace('\\n', '\n')
        return template.replace('*sent 0*', text).strip()
...
def generate_sentemb_prompt(data_point, tokenizer, cutoff_len, template, prefix='input'):
    ...
    if isinstance(template, list):
        # print("process_qwen_template", process_qwen_template(template, input))
        return process_qwen_template(template, input)
    else:
        template = template.replace('_', ' ').replace('*sep+*', '')\
                                             .replace('*cls*', '').replace('\\n', '\n')
...
def train(
    ...
    if 'llava' in base_model: # 检查 Phi-3 模型是否在 base_model 中
        ...
        if load_kbit == 4: # 'Phi-3' in base_model, 加载 4-bit 量化的模型
            model = AutoModelForCausalLM.from_pretrained(
                base_model,
                quantization_config=BitsAndBytesConfig(
                    load_in_4bit=True,
        ...
        # 直接加载完整的Qwen2.5-VL模型，不分离language_model
        # 这样可以保持完整的模型结构，避免保存后缺少顶层结构的问题
        if load_kbit == 4:
    ...
    if 'Qwen' in base_model:
        if mask_embedding_sentence_template:
            qwen2_5_vl_template[0]["content"][0]["text"] = mask_embedding_sentence_template
            mask_embedding_sentence_template = qwen2_5_vl_template
    ...
    def tokenize(prompt, add_eos_token=True, label_prompt=None, neg_prompt=None):
        # 为Qwen模型特殊处理tokenization
        if 'Qwen' in base_model and isinstance(prompt, list):
            # 使用apply_chat_template将列表格式转换为字符串
            prompt = tokenizer.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True)
            # print("prompt = ", prompt)
        
        # 处理label_prompt
        if label_prompt and 'Qwen' in base_model and isinstance(label_prompt, list):
            label_prompt = tokenizer.apply_chat_template(label_prompt, tokenize=False, add_generation_prompt=True)
        
        # 处理neg_prompt
        if neg_prompt and 'Qwen' in base_model and isinstance(neg_prompt, list):
            neg_prompt = tokenizer.apply_chat_template(neg_prompt, tokenize=False, add_generation_prompt=True)
        ...
        if 'Phi-3' in base_model:
            ...
            target_modules = [
                [f'model.language_model.layers.{i}.mlp.gate_proj',
                 ...
                 f'model.language_model.layers.{i}.self_attn.o_proj' ] for i in range(28)
            ]
            target_modules = sum(target_modules, [])
            print("Qwen LoRA target modules:", target_modules)
        else:
    ...
    swanlab_callback = SwanLabCallback(
        ...
        experiment_name="QwenVL-qlora-embedding",
        # config=config,
        config={
            "r": lora_r,  # 低秩矩阵的秩
            "lora_alpha": lora_alpha,  # LoRA 的缩放因子
